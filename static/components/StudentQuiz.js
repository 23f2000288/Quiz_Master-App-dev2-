  ...
  methods: {
    async fetchQuizData() {
      try {
        ...
        const quizDueDate = new Date(this.quiz.due_date);
  
        if (this.quiz.is_completed || currentDate > quizDueDate) {
          this.moveToCompletedQuizzes();
        } else {
          this.initTimer();
        }
    ...
    async moveToCompletedQuizzes() {
      try {
        const response = await fetch(`/api/student/quiz/${this.quiz.id}/complete`, {
          method: 'POST',
          headers: {
            'Authentication-Token': this.authToken,
          }
        ...
        console.error('Error moving quiz to completed:', error);
      ...
      async submitQuiz() {
        ...
        try {
          response = await fetch(`/api/student/quiz/${this.quiz.id}/submit`, {
            method: 'POST',
            headers: {
              'Authentication-Token': this.authToken,
          ...
          if (response.ok) {
            this.$emit('quiz-completed', this.quiz.id); // Emit the event
            this.$router.push('/'); // Go back to the quiz list or replace `/`
          } else {
