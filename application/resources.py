...
from .models import Subject, Chapter, Quiz, Question, User, db, Role, Score, UserAnswer
...
## From here to the last line of the code refers to User Side
...
class StudentQuizResource(Resource):
    ...
    def get(self):
        ...
        result = [{
            ...
            'status': 'upcoming' if quiz.date_of_quiz > today else 'completed'
        } for quiz, is_completed in quizzes]
        
        return result

api.add_resource(StudentQuizResource, '/student/quizzes')

class StudentQuizRetakeResource(Resource):
    @auth_required('token')
    @roles_required('stud')
    def post(self, quiz_id):
...
class StudentQuizResultResource(Resource):
    ...
    def get(self, quiz_id):
        ...
        results = []
        for question in questions:
            is_correct = False  # Default is False
            
            if user_score:  # If user has attempted the quiz
                is_correct = (user_score.total_scored > 0)  # Check if the score is greater than 0
            else:
                user_answer = None  # Or handle the case where the question wasn't answered
                is_correct = False
...
api.add_resource(StudentQuizResultResource, '/student/quiz/<int:quiz_id>/results')

class StudentQuizCompleteResource(Resource):
    @auth_required('token')
    @roles_required('stud')
    def post(self, quiz_id):
        quiz = Quiz.query.get_or_404(quiz_id)
        ...
        try:
            db.session.commit()
            return {"message": "Quiz marked as completed", "status": "completed"}, 200
        except Exception as e:
            db.session.rollback()
            return {"message": f"Error completing quiz: {str(e)}", "status": "error"}, 500

api.add_resource(StudentQuizCompleteResource, '/student/quiz/<int:quiz_id>/complete')
...
class StudentQuizResultResource(Resource):
    ...
    def get(self, quiz_id):
        ...
        for question in questions:
            ...
            if user_score:
                # Determine if the answer was correct (assuming total_scored reflects correct answers)
                is_correct = (user_score.total_scored > 0)  # Check if the score is greater than 0
            else:
                is_correct = False
